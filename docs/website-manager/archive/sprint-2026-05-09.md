...
- **Deployment:** **Live** — PM heeft `git commit` (`75977ba`) + `git push origin main` voltooid op 8 mei 2026 16:28. **GitHub Actions:** `pages-build-deployment` run **25566957636** — succes (50s). Live-check: `curl https://www.vlwarmte.nl/logo-varianten.html` → meta-refresh + canonical naar `/` ✓.
