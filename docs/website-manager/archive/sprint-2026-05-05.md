...
### Taak 6: GA4 custom event voor uitgaande Facebook-klikken `[GOEDGEKEURD]`

**Bron:** PM / gebruiker (1 mei 2026) — expliciet goedgekeurd na voorstel; kleine analytics-taak buiten de oorspronkelijke vijf, bewust toegestaan.

**Prioriteit:** Normaal  
**Type:** analytics (geen nieuwe URL)

**Actie:** In `assets/js/main.js` bij `DOMContentLoaded` een lichte click-listener registreren die alleen vuurt als `gtag` bestaat en de geklikte `<a href>` naar `facebook.com` wijst. Eventnaam: **`facebook_outbound`**. Parameters: `link_url` (volledige href), `link_text` (trimmed anchor-tekst, max. 100 tekens). Geen dubbele events bij middle-click of programmatisch — alleen de gebruikers-click bubble is voldoende. **`prijsindicatie.html`** laadt `main.js` deferred (na `ga-deferred.js`) zodat footer-Facebook daar hetzelfde event triggert en de mobiele nav hetzelfde script gebruikt als de rest van de site.

**Succescriterium:** In GA4 DebugView (of realtime) zichtbaar bij een testklik op een Facebook-link op de site; geen console-errors op pagina’s zonder `gtag` (listener no-op).
...
- **Facebook-pagina op de site (1 mei, PM + team):** Link staat in de **footer** op alle root-pagina’s (verwacht patroon, geen concurrentie met primaire CTA’s) en extra onder **bedrijfsgegevens** op `contact.html` waar hoog-intent-bezoekers toch al vertrouwen checken. **Niet** in de header naast offerte/nav — daar lekt aandacht van de leadflow. URL: `https://www.facebook.com/1068733929659423` (zelfde page-id als Meta Graph). Op `index.html` ook `sameAs` in `LocalBusiness`-JSON-LD voor consistente bedrijfssignalen. **Meting:** custom event `facebook_outbound` (zie Taak 6); optioneel later als *key event* in GA4-admin markeren.
...
**Taak 6 — GA4 `facebook_outbound`:** klaar. Delegated click listener in `assets/js/main.js` (alleen bij `facebook.com` + aanwezig `gtag`); `prijsindicatie.html` laadt `main.js` mee.
